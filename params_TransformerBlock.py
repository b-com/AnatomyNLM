# params_TransformerBlock.py – Software AnatomyNLM
...
"""
Created on Tue Sep 26 16:08:36 2023
...
def main(d_k,d_v,d_e,M,d_f,encoder):
    ...
    if encoder:
        transformer = TransformerDecoder(intermediate_dim=d_f, num_heads=M)
    else:
        transformer = TransformerEncoder(intermediate_dim=d_f, num_heads=M)
        
    input = Input(shape=(10, d_e)) # 10 is only an example of the sequence length
                                   # you can use any other number of tokens
                                   # this doesn't affect the number of trainable 
                                   # params of the transformer block
    ...
    num_params=params_calc.num_params_transformerBlock(d_k,d_v,d_e,M,d_f)
...
if __name__ == '__main__':
    ...
    main(d_k,d_v,d_e,M,d_f,encoder)
